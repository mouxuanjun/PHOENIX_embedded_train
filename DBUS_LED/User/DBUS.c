...
void RemoteDataProcess(rc_info_t *rc_ctrl, uint8_t rx_data[18]){
    rc_ctrl -> rc.ch0 = ((int16_t)rx_data[0]) | ((int16_t)rx_data[1] << 8) & 0x07ff;
	rc_ctrl -> rc.ch0 -= 1024;
    rc_ctrl -> rc.ch1 = ((int16_t)rx_data[1] >> 3) | ((int16_t)rx_data[2] << 5) & 0x07ff;
	rc_ctrl -> rc.ch1 -=1024;
    rc_ctrl -> rc.ch2 = ((int16_t)rx_data[2] >> 6) | ((int16_t)rx_data[3] << 2) | ((int16_t)rx_data[4] << 10) & 0x07ff;
	rc_ctrl -> rc.ch2 -= 1024;
    rc_ctrl -> rc.ch3 = ((int16_t)rx_data[4] >> 1) | ((int16_t)rx_data[5] << 7) & 0x07ff;
	rc_ctrl -> rc.ch3 -= 1024;
	rc_ctrl -> rc.s1 = ((int16_t)rx_data[5] >> 4 & 0x0003);
    rc_ctrl -> rc.s2 = ((int16_t)rx_data[5] >> 4 & 0x000C) >> 2;
	rc_ctrl -> rc.wheel = rx_data[16] | (rx_data[17] << 8) - 1024;
