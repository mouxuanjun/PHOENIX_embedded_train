...
//	    pid_init(&motor_pitch.Angle_pid,0,0,0,0,0);
    int16_t w = 0;
    int16_t R = 0;
//    int16_t Vx = rc_ctrl.rc.ch3/80.0f;
//	int16_t Vy = rc_ctrl.rc.ch2/80.0f;
    int16_t Vx = 80.0f;
    int16_t Vy = 80.0f;
    motor_chassis[0].set_speed = -0.707f * Vx + 0.707f * Vy+w*R;
	motor_chassis[1].set_speed = -0.707f * Vx - 0.707f * Vy+w*R;
	motor_chassis[2].set_speed =  0.707f * Vx - 0.707f * Vy+w*R;
	motor_chassis[3].set_speed =  0.707f * Vx + 0.707f * Vy+w*R;
	for(int i = 0; i < 4; i++)
	{
		pid_init(&motor_chassis[i].Speed_pid,0,0,0, 0,1000,18000);
		PID_Calc_Speed(&motor_chassis[i].Speed_pid,motor_chassis[i].set_speed,motor_chassis[i].motor_speed);
	}
    CAN_cmd_chassis(motor_chassis[0].motor_speed,motor_chassis[1].motor_speed,motor_chassis[2].motor_speed,motor_chassis[3].motor_speed);
	osDelay(1);
//}
//		} 
//	}		
