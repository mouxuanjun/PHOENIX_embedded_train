...
typedef struct
{
	...
	uint32_t attack_flag : 1;
	uint32_t rate_flag : 1;
	uint32_t time_flag : 1;
	uint32_t heat_flag : 1;
	uint32_t tunnel_flag : 1;
	uint32_t bullet_flag : 1;
	uint32_t chassis_level_flag : 1;
	uint32_t cross_state_flag : 1;
...
typedef struct
{
	...
	loader_mode_e loader_mode;				 // 发射模式设置
	...
	Heat_mode_e heat_control;
	time_mode_e time_mode;
	//float pitch_angle;
	//float yaw_angle;
	tunnel_mode_e tunnel_mode;
	int rest_bullets;						//自算弹舱剩余子弹
	uint16_t shoot_heat;					//自算枪口热量
	uint16_t shoot_heat_limit;				//枪口热量限制
	uint16_t chassis_power_robot_level;
	uint16_t cross_state;

	// 上一次的模式，用于flag判断
	chassis_mode_e chassis_last_mode;
	loader_mode_e loader_last_mode;
	friction_mode_e friction_last_mode;
	time_mode_e time_last_mode;
	// lid_mode_e lid_last_mode;
	Chassis_Power_Data_s Chassis_last_Power_Data;
	vision_mode_e vision_last_mode;
	attack_mode_e attack_last_mode;
	Heat_mode_e heat_last_control;
	//float pitch_last_angle;
	//float yaw_last_angle;
	tunnel_mode_e tunnel_last_mode;
	int last_rest_bullets;
	uint16_t last_shoot_heat;
	uint16_t chassis_power_last_robot_level;
	uint16_t cross_last_state;
